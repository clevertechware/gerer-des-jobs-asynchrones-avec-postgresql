...
- **`main.go`**: composition root — wires config → pgxpool → `PGTxManager` → `JobRepository` →
  usecases → web handlers + worker, then runs the HTTP server, worker loop, and reaper concurrently,
...
- Repository tests use `testify/suite` (`PostgresSuite` in `suite_test.go`) against a real Postgres
  ...
  test binary run (`sync.Once`) and reused across the suite for speed.
